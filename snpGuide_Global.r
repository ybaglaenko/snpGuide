...
library(httr)
library(jsonlite)
library(tidyverse) # for ease of use
library(Biostrings) # for reading in fasta files
library(magrittr) ## %<>% functionality
...
snpGuide <- function(rsid,
    ...
    min_editing_window=3,
    max_editing_window=10,
...
#Reference guides
guides_ref <- guides_design_func(reference_seq, alleles[1], alleles[2], "reference")
...
seq_alt <- DNAString(paste0(str_sub(as.character(seq), 1, 20), 
                 allele2,
                 str_sub(as.character(seq), 22, 41)))
    
guide_alt <- guides_design_func(seq_alt, allele2, alleles[1], paste0("alternative",i))

guides <- rbind(guides,guide_alt)}

guides %<>% forecast_func %>% mutate(snp = data$name)
